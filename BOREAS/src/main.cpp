...
Serial.println("DIFERENCIA DE CO2:"); 
Serial.println(co2_diference); 
Decay63_percentage = co2_outdoor + (euler_number_negative * co2_diference);
Serial.println("Decay"); 
Serial.println(Decay63_percentage); 
ventilation_rate = 1;
Serial.println("RATIO DE VENTILACION"); 
Serial.println(ventilation_rate); 
Total_order_loss_rate = ventilation_rate + 0.62 + 0.3;
Serial.println("Total order loss rate"); 
Serial.println(Total_order_loss_rate); 
Net_emission_Rate = Quanta_Exalation_rate_of_posible_infected *(1-Mask_eficiency*1)*1;
Serial.println("Emission neta"); 
Serial.println(Net_emission_Rate); 
float prueba = 0;
float prueba2 = 0 ;
float prueba3 = 0;
prueba = (Net_emission_Rate/Total_order_loss_rate/volume);
prueba2 = 1-(1-Total_order_loss_rate/1);
prueba3 = (1-pow(euler_number,Total_order_loss_rate));
average_quanta_concentration = prueba*prueba2*prueba3;
Serial.println("average quanta concentration"); 
Serial.println(average_quanta_concentration); 
quanta_inhaled_per_person = average_quanta_concentration * Breathing_rate * 1 * (1 - (Mask_eficiency * 1));
Serial.println("Quanta inalda por persona"); 
Serial.println(quanta_inhaled_per_person); 
